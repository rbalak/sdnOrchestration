...
var run = function(){
	...
	var utilHighQuery = "SELECT count(value) FROM trafficUtilPct  WHERE time > now() - 420s and value >= "  + config.utilHighThreshold;
	...
	var utilLowQuery = "SELECT count(value) FROM trafficUtilPct  WHERE time > now() - 420s and value <= "  + config.utilLowThreshold;
